...
export const MemoryView: React.FC<MemoryViewProps> = ({ memoryRecords, onNavigateToLedger }) => {
  ...
  return (
    <div className="factory-enter factory-grid space-y-6 pb-12">
      <div className="factory-panel factory-scan rounded-xl border-b border-neutral-800 p-5 flex items-start justify-between gap-4">
        <div><div className="text-[10px] font-mono text-purple-300 tracking-[0.2em] uppercase">Memory / Knowledge Control Plane</div><h1 className="text-lg font-bold font-mono text-neutral-100 uppercase tracking-wide flex items-center gap-2 mt-1"><Brain className="h-5 w-5 text-purple-400" /><span>Epistemic Memory & Knowledge</span></h1><p className="text-xs text-neutral-400 font-mono mt-1">Governed Obsidian and Claude ingestion with manifest, review, drift detection and atomic commit.</p></div>
        <button onClick={() => setShowIngest((v) => !v)} className="shrink-0 px-3 py-2 rounded-lg border border-cyan-500/30 bg-cyan-500/10 text-cyan-300 text-xs font-mono font-bold flex items-center gap-2 hover:bg-cyan-500/15"><Upload className="h-4 w-4" />{showIngest ? 'Close Ingestion' : 'Ingest Knowledge'}</button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="factory-panel-raised rounded-xl p-4"><div className="text-[10px] text-neutral-500 font-mono uppercase">Factory Knowledge</div><div className="text-2xl font-bold text-neutral-100 font-mono mt-1">{knowledgeRecords.length}</div><div className="text-[10px] text-neutral-500 font-mono">committed chunks</div></div>
        <div className="factory-panel-raised rounded-xl p-4"><div className="text-[10px] text-neutral-500 font-mono uppercase flex items-center gap-1"><FileText className="h-3 w-3 text-blue-400" /> Obsidian</div><div className="text-2xl font-bold text-blue-300 font-mono mt-1">{obsidianCount}</div><div className="text-[10px] text-neutral-500 font-mono">vault-derived records</div></div>
        <div className="factory-panel-raised rounded-xl p-4"><div className="text-[10px] text-neutral-500 font-mono uppercase flex items-center gap-1"><MessageSquare className="h-3 w-3 text-amber-400" /> Claude</div><div className="text-2xl font-bold text-amber-300 font-mono mt-1">{claudeCount}</div><div className="text-[10px] text-neutral-500 font-mono">distilled knowledge records</div></div>
      </div>

      {showIngest && <div className="factory-panel rounded-xl p-5 space-y-4">
        <div className="flex items-center justify-between gap-3"><div><div className="flex items-center gap-2 text-sm font-bold font-mono text-neutral-100"><Sparkles className="h-4 w-4 text-cyan-400" /> Knowledge Source Adapter</div><div className="text-[11px] text-neutral-500 font-mono mt-1">Discovery is read-only. Nothing enters Memory until the governed transaction reaches atomic ingestion.</div></div><button onClick={() => void loadItems()} disabled={busy} className="px-2.5 py-1.5 rounded-lg border border-neutral-700 text-neutral-300 text-[10px] font-mono flex items-center gap-1 hover:border-cyan-500/30"><RefreshCw className="h-3 w-3" /> REFRESH</button></div>
        ...
        {preview && <div className="rounded-lg border border-neutral-800 bg-neutral-950 p-3 factory-scan"><div className="text-[10px] text-cyan-300 font-mono mb-2">PREVIEW · {preview.item.path}</div><pre className="max-h-52 overflow-y-auto whitespace-pre-wrap text-[11px] text-neutral-300 font-mono">{preview.content}</pre></div>}
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-neutral-800 pt-3"><span className="text-[10px] font-mono text-neutral-500 flex items-center gap-1"><Hash className="h-3 w-3 text-cyan-400" /> Server-side SHA-256 manifest identity · human approval required</span><button disabled={busy || !selectedItems.length} onClick={() => void createManifest()} className="px-4 py-2 rounded-lg bg-cyan-500 text-neutral-950 text-xs font-mono font-bold disabled:opacity-50 hover:bg-cyan-300">{busy ? 'WORKING…' : 'CREATE MANIFEST'}</button></div>
        {transaction && <div className="factory-panel-raised rounded-xl border-amber-500/20 bg-amber-500/5 p-4 space-y-3"><div className="flex items-center justify-between gap-3"><div className="text-xs font-mono text-neutral-200 font-bold flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-amber-300" /> {transaction.transactionId}</div><span className="px-2 py-1 rounded border border-amber-500/30 text-amber-300 text-[10px] font-mono font-bold">{transaction.state}</span></div><div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-[10px] font-mono"><div className="rounded bg-neutral-950 p-2"><span className="text-neutral-500 block">ITEMS</span><span className="text-neutral-200">{transaction.manifest.length}</span></div><div className="rounded bg-neutral-950 p-2"><span className="text-neutral-500 block">MANIFEST HASH</span><span className="text-cyan-300 break-all">{transaction.manifestHash}</span></div><div className="rounded bg-neutral-950 p-2"><span className="text-neutral-500 block">VERSION</span><span className="text-neutral-200">{transaction.manifest.map((m) => m.version).join(', ')}</span></div></div>{stateAction && <button disabled={busy} onClick={() => void stateAction()} className="w-full px-4 py-2 rounded-lg border border-amber-500/40 bg-amber-500/10 text-amber-200 text-xs font-mono font-bold disabled:opacity-50 hover:bg-amber-500/15">{busy ? 'PROCESSING…' : stateLabel}</button>}</div>}
        {status && <div className="text-xs font-mono text-cyan-300 border-t border-neutral-800 pt-3">{status}</div>}
      </div>}

      <div className="factory-panel rounded-xl p-3 flex flex-wrap gap-2 text-xs font-mono"><span className="text-[10px] text-neutral-500 uppercase tracking-widest self-center mr-1">Record Class</span>{['ALL', 'RAW_OBSERVATION', 'EVIDENCE', 'MEMORY', 'DISTILLED_KNOWLEDGE', 'MODEL_OUTPUT'].map((cat) => <button key={cat} onClick={() => setSelectedType(cat)} className={`px-3 py-1.5 rounded-lg border transition ${selectedType === cat ? 'bg-purple-500/10 text-purple-200 border-purple-500/40 font-bold' : 'border-neutral-800 bg-neutral-950/60 text-neutral-400 hover:text-neutral-200'}`}>{cat}</button>)}</div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-5 space-y-2 font-mono text-xs"><div className="flex items-center justify-between"><span className="font-bold text-neutral-400 uppercase tracking-widest">Memory Records</span><span className="text-[10px] text-neutral-600">{filtered.length} visible</span></div><div className="space-y-2 max-h-[700px] overflow-y-auto pr-1">{filtered.map((rec) => { const isSelected = selectedRecord?.id === rec.id; return <div key={rec.id} onClick={() => setSelectedRecord(rec)} className={`p-3 rounded-xl border transition cursor-pointer space-y-1.5 ${isSelected ? 'border-purple-500/60 bg-neutral-900/90 ring-1 ring-purple-500/30 factory-node-active' : 'border-neutral-800 bg-neutral-950/60 hover:border-neutral-700'}`}><div className="flex items-center justify-between gap-2"><span className="font-bold text-neutral-200 truncate">{rec.source}</span><span className={`text-[9px] px-1.5 py-0.2 rounded border ${getTypeBadge(rec.type)}`}>{rec.type}</span></div><p className="text-neutral-300 font-sans text-xs line-clamp-2">{rec.content}</p><div className="pt-1.5 border-t border-neutral-900 flex items-center justify-between text-[10px] text-neutral-400"><span>Confidence: <strong className="text-emerald-400">{rec.confidence}%</strong></span><span>{new Date(rec.timestamp).toLocaleTimeString()}</span></div></div>; })}</div></div>
        <div className="lg:col-span-7 space-y-4 font-mono text-xs">{selectedRecord ? <div className="factory-panel-raised rounded-xl p-5 space-y-4"><div className="flex items-center justify-between border-b border-neutral-800 pb-3 gap-3"><div><span className="text-[10px] text-neutral-500 block">{selectedRecord.id}</span><h2 className="text-sm font-bold text-neutral-100">{selectedRecord.source}</h2></div><span className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${getTypeBadge(selectedRecord.type)}`}>{selectedRecord.type}</span></div><div className="p-3.5 rounded-lg bg-neutral-950 border border-neutral-800 space-y-2"><span className="text-[11px] text-neutral-400 font-bold block uppercase">Knowledge Payload</span><p className="text-neutral-200 font-sans text-xs leading-relaxed whitespace-pre-wrap">{selectedRecord.content}</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px]"><div className="p-2.5 rounded bg-neutral-950 border border-neutral-800"><span className="text-neutral-500 block">Source Origin</span><span className="text-neutral-200 font-semibold">{selectedRecord.source}</span></div><div className="p-2.5 rounded bg-neutral-950 border border-neutral-800"><span className="text-neutral-500 block">Trace</span><span className="text-cyan-300 font-semibold">{selectedRecord.provenance.traceId}</span></div></div><div className="space-y-1.5"><span className="text-[11px] text-neutral-400 uppercase flex items-center gap-1"><Link2 className="h-3 w-3" /> Provenance Chain</span><div className="flex flex-wrap gap-1.5">{selectedRecord.provenance.chain.map((item) => <span key={item} className="px-2 py-0.5 rounded bg-neutral-950 border border-neutral-800 text-[10px] text-cyan-300">{item}</span>)}</div></div><div className="space-y-1.5"><span className="text-[11px] text-neutral-400 uppercase">Tags & Indices</span><div className="flex flex-wrap gap-1.5">{selectedRecord.tags.map((t) => <span key={t} className="px-2 py-0.5 rounded bg-neutral-950 border border-neutral-800 text-[10px] text-purple-300">#{t}</span>)}</div></div>{selectedRecord.associatedEventId && <div className="pt-2 border-t border-neutral-800"><button onClick={() => onNavigateToLedger(selectedRecord.associatedEventId!)} className="text-cyan-400 hover:underline font-bold flex items-center gap-1"><Database className="h-3 w-3" /> Open ledger event {selectedRecord.associatedEventId}</button></div>}<div className="text-[10px] text-neutral-500 flex items-center gap-1"><CheckCircle2 className="h-3 w-3 text-emerald-400" /> Identity and provenance metadata retained with the memory record.</div></div> : <div className="p-12 text-center text-neutral-500 font-mono text-xs">Select a memory record.</div>}</div>
      </div>
    </div>
  );
};
