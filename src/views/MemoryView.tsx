...
import { Brain, Database, Sparkles, CheckCircle2, Upload, FileText, MessageSquare, Hash, Link2 } from 'lucide-react';
...
export const MemoryView: React.FC<MemoryViewProps> = ({ memoryRecords, onNavigateToLedger }) => {
  ...
  const ingest = async () => {
    ...
    if (!sourceName.trim() || chunks.length === 0) { setStatus('Source name and knowledge content are required.'); return; }
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const memory: MemoryRecord = {
          ...
          provenance: { sourceId: `doc-${hash.slice(0, 16)}`, traceId: `knowledge-${hash.slice(0, 12)}`, chain: [source, 'Document Ingestion', 'SHA-256 Content Identity', 'Markdown Chunking', source === 'CLAUDE' ? 'DISTILLED_KNOWLEDGE' : 'RAW_OBSERVATION'] },
          timestamp: now,
          confidence: source === 'CLAUDE' ? 85 : 95,
          relevance: 100,
          tags: [...baseTags, `chunk:${i + 1}/${chunks.length}`, `content-hash:${hash.slice(0, 12)}`],
        };
        const response = await fetch('/api/memory', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(memory) });
      ...
      setContent(''); setPath('');
    } catch (error) { setStatus(error instanceof Error ? error.message : 'Knowledge ingestion failed.'); }
    ...
    <div className="space-y-6 pb-12">
      <div className="border-b border-neutral-800 pb-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-lg font-bold font-mono text-neutral-100 uppercase tracking-wide flex items-center gap-2"><Brain className="h-5 w-5 text-purple-400" /><span>Epistemic Memory & Knowledge</span></h1>
            <p className="text-xs text-neutral-400 font-mono mt-1">Claude knowledge and Obsidian vault material enter Factory Memory as provenance-bound records — never as opaque context.</p>
          </div>
      ...
      {showIngest && <div className="rounded-xl border border-cyan-500/20 bg-neutral-900/70 p-5 space-y-4">
        ...
        <div className="text-[11px] text-neutral-500 font-mono">Paste exported Markdown/text from Obsidian or Claude. The Factory hashes and chunks it before committing each chunk to Memory.</div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="text-[10px] font-mono text-neutral-400">SOURCE TYPE<select value={source} onChange={(e) => setSource(e.target.value as IngestSource)} className="mt-1 w-full bg-neutral-950 border border-neutral-800 rounded-lg p-2 text-xs text-neutral-200"><option value="OBSIDIAN">OBSIDIAN</option><option value="CLAUDE">CLAUDE</option></select></label>
          <label className="text-[10px] font-mono text-neutral-400">SOURCE NAME<input value={sourceName} onChange={(e) => setSourceName(e.target.value)} className="mt-1 w-full bg-neutral-950 border border-neutral-800 rounded-lg p-2 text-xs text-neutral-200" /></label>
          <label className="text-[10px] font-mono text-neutral-400">VAULT / EXPORT PATH<input value={path} onChange={(e) => setPath(e.target.value)} placeholder="e.g. Factory/Claude/architecture.md" className="mt-1 w-full bg-neutral-950 border border-neutral-800 rounded-lg p-2 text-xs text-neutral-200" /></label>
          <label className="text-[10px] font-mono text-neutral-400">TAGS<input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="architecture, agents, memory" className="mt-1 w-full bg-neutral-950 border border-neutral-800 rounded-lg p-2 text-xs text-neutral-200" /></label>
        </div>
        ...
        <div className="flex items-center justify-between gap-3"><span className="text-[10px] font-mono text-neutral-500 flex items-center gap-1"><Hash className="h-3 w-3" /> SHA-256 identity + provenance chain generated client-side</span><button disabled={ingesting} onClick={ingest} className="px-4 py-2 rounded-lg bg-cyan-500 text-neutral-950 text-xs font-mono font-bold disabled:opacity-50">{ingesting ? 'COMMITTING…' : 'COMMIT TO FACTORY MEMORY'}</button></div>
