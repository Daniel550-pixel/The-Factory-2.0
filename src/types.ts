...
export interface ExecutionContext {
  ...
  authorization?: {
    authorized: boolean;
    authorizer: string;
    method: 'AUTOMATIC_POLICY' | 'HUMAN_APPROVAL' | 'REPLAY_OVERRIDE';
    timestamp: string;
  };
...
export interface Agent {
  ...
  name: string;
  description: string;
  type: 'SUPERVISOR' | 'SYSTEMS_ARCHITECT' | 'SECURITY_OFFICER' | 'INFRASTRUCTURE_OPS' | 'FINANCIAL_RISK' | 'LOGISTICS_GEO' | 'VERIFIER' | 'COMPLIANCE';
  status: 'ACTIVE' | 'IDLE' | 'BUSY' | 'PAUSED' | 'DEGRADED';
  capabilities: string[];
  tools: string[];
  modelProvider: string;
  confidence: number;
  riskProfile: 'LOW' | 'MEDIUM' | 'HIGH' | 'STRICT';
  currentTask?: string;
...
export interface ApprovalRequest {
  ...
  requestedAction: string;
  timestamp: string;
  status: 'PENDING' | 'APPROVED' | 'DENIED' | 'MORE_EVIDENCE_REQUESTED';
  reviewer?: string;
...
export interface ToolDefinition {
  ...
  executionMode: 'SANDBOXED' | 'LOCAL_KERNEL' | 'API_GATEWAY' | 'HOST_ISOLATED';
  status: 'AVAILABLE' | 'RESTRICTED' | 'OFFLINE';
  owningAgentId: string;
  auditStatus: 'COMPLIANT' | 'NEEDS_REVIEW';
...
export interface ProviderDefinition {
  id: string;
  name: string;
  type: 'GEMINI' | 'OPENAI' | 'ANTHROPIC' | 'LOCAL_KERNEL';
  model: string;
...
export interface ProductIntegration {
  ...
  name: string;
  adapter: string;
  status: 'CONNECTED' | 'DISCONNECTED' | 'DEGRADED' | 'STANDBY';
  capabilities: string[];
  eventsCount: number;
  executionsCount: number;
...
export interface SystemStatus {
  runtimeStatus: 'OPERATIONAL' | 'DEGRADED' | 'EMERGENCY_LOCKDOWN';
  runtimeMode: RuntimeMode;
  ledgerIntegrity: 'VERIFIED' | 'TAMPER_DETECTED' | 'VERIFYING';
  totalEvents: number;
  activeAgents: number;
  ...
  securityState: 'ENFORCED' | 'ALERT' | 'AUDIT_MODE';
...
export interface NavigationTab {
  id: string;
  label: string;
  section: 'MAIN' | 'RUNTIME' | 'TRUST' | 'STATE' | 'OPERATIONS' | 'INTELLIGENCE' | 'PRODUCTS' | 'SYSTEM';
  badge?: number | string;
}
