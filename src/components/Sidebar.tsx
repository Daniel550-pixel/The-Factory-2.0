...
export const Sidebar: React.FC<SidebarProps> = ({ currentTab, onTabChange, status }) => {
  ...
  return (
    <aside className="flex h-[calc(100vh-64px)] w-[248px] shrink-0 flex-col border-r border-white/[0.06] bg-[#080a0c]/90 backdrop-blur-xl select-none">
              ...
              {section.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative flex w-full items-center justify-between rounded-md border px-2.5 py-2 text-left transition-all duration-150 ${
                      active
                  ...
                  >
                    ...
                    {item.badge !== undefined && item.badge !== null && item.badge > 0 && (
                      <span className={`ml-2 rounded border px-1.5 py-0.5 font-mono text-[8px] leading-none ${item.badgeTone === 'amber' ? 'border-amber-400/15 bg-amber-400/10 text-amber-300' : active ? 'border-cyan-400/15 bg-cyan-400/10 text-cyan-200' : 'border-white/[0.05] bg-white/[0.035] text-neutral-600'}`}>
                        {item.badge}
                      </span>
                    )}
                  </button>
